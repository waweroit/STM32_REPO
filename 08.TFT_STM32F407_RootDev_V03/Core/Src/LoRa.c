...
LoRa newLoRa(){
	...
	new_LoRa.spredingFactor        = SF_9      ; // dla anteny 21cm 5 DBa powinno byc SF_9 lub 10 , dla anteny 11cm SF_7 jest OK
	new_LoRa.bandWidth			   = BW_125KHz ; // Szerokość pasma 125 kHz
	new_LoRa.crcRate               = CR_4_5    ; // Korekcja błędów 4/5
	new_LoRa.power				   = POWER_17db; // Moc wyjściowa 20 dBm // Rozważ obniżenie mocy do 17-18 dBm, aby ograniczyć interferencje, jeśli nie potrzebujesz maksymalnego zasięgu.
	new_LoRa.overCurrentProtection = 100       ; // Ochrona przed przeciążeniem 100 mA
	new_LoRa.preamble			   = 8         ; // Długość preambuły 8 // Dłuższa preambuła zwiększa stabilność przy dużych odległościach // Krótka preambuła przyspiesza połączenia
...
//	return read_data;
//}
// nowa wygenerowana przez AI
uint8_t LoRa_read(LoRa* _LoRa, uint8_t address) {
    uint8_t addr = address & 0x7F;  // Bit odczytu w adresie
    uint8_t value = 0;

    HAL_GPIO_WritePin(_LoRa->CS_port, _LoRa->CS_pin, GPIO_PIN_RESET);  // Aktywacja
    HAL_SPI_Transmit(_LoRa->hSPIx, &addr, 1, HAL_MAX_DELAY);
    DelayUS(5);  // Krótkie opóźnienie dla stabilności
    HAL_SPI_Receive(_LoRa->hSPIx, &value, 1, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(_LoRa->CS_port, _LoRa->CS_pin, GPIO_PIN_SET);  // Dezaktywacja
...
// nowa wygenerowana przez AI
void LoRa_write(LoRa* _LoRa, uint8_t address, uint8_t value) {
    uint8_t addr = address | 0x80;  // Bit zapisu w adresie

    HAL_GPIO_WritePin(_LoRa->CS_port, _LoRa->CS_pin, GPIO_PIN_RESET);  // Aktywacja układu
    HAL_SPI_Transmit(_LoRa->hSPIx, &addr, 1, HAL_MAX_DELAY);
    DelayUS(5);  // Krótkie opóźnienie
...
void LoRa_BurstWrite(LoRa* _LoRa, uint8_t address, uint8_t *value, uint8_t length) {
    ...
    for (uint8_t i = 0; i < length; i++) {
        HAL_SPI_Transmit(_LoRa->hSPIx, &value[i], 1, HAL_MAX_DELAY);
        DelayUS(5);  // Krótkie opóźnienie dla stabilności transmisji
    }
...
// optimized lora transmit
uint8_t LoRa_transmit(LoRa* _LoRa, uint8_t* data, uint8_t length, uint16_t timeout) {
    // Zapamiętaj bieżący tryb pracy modułu
    int previousMode = _LoRa->current_mode;

    // Przejdź do trybu oczekiwania
    LoRa_gotoMode(_LoRa, STNBY_MODE);

    // Ustaw wskaźnik FIFO i długość pakietu
    uint8_t baseAddr = LoRa_read(_LoRa, RegFiFoTxBaseAddr);
    LoRa_write(_LoRa, RegFiFoAddPtr, baseAddr);
    LoRa_write(_LoRa, RegPayloadLength, length);
    LoRa_BurstWrite(_LoRa, RegFiFo, data, length);

    // Przejdź do trybu transmisji
    LoRa_gotoMode(_LoRa, TRANSMIT_MODE);
    ...
    while (timeout--) {
        ...
        // Sprawdzenie, czy transmisja zakończona
        if (irqFlags & 0x08) {
            LoRa_write(_LoRa, RegIrqFlags, 0xFF);  // Wyczyść flagę IRQ
            LoRa_gotoMode(_LoRa, previousMode);    // Przywróć poprzedni tryb pracy
            return 1;  // Zwróć sukces
        }

        DelayUS(5);  // Krótkie opóźnienie dla stabilności
    }

    // Timeout: Przywróć poprzedni tryb pracy i zwróć błąd
    LoRa_gotoMode(_LoRa, previousMode);
    return 0;
}
