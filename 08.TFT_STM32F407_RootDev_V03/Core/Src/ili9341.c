...
void lcdDisplayTextWithScrollPage(const char *text, uint32_t displayTimeMs, uint16_t startTextXposition, uint16_t startTextYposition, uint16_t textColor, uint16_t backgroundColor)
{
    uint16_t screenWidth = ILI9341_PIXEL_HEIGHT;  // Szerokość ekranu
    uint16_t screenHeight = ILI9341_PIXEL_WIDTH;  // Wysokość ekranu

    uint16_t charWidth = lcdGetTextFont()->Width;  // Szerokość pojedynczego znaku
    uint16_t charHeight = lcdGetTextFont()->Height; // Wysokość pojedynczego znaku

    uint16_t charsPerLine = screenWidth / charWidth; // Maksymalna liczba znaków na linię
    uint16_t maxLines = screenHeight / charHeight;  // Maksymalna liczba linii na ekranie

    uint16_t currentX = startTextXposition;  // Pozycja X na ekranie
    uint16_t currentY = startTextYposition;  // Pozycja Y na ekranie
    char buffer[charsPerLine + 1];  // Bufor dla jednej linii (+1 dla null-terminatora)

    const char *currentChar = text;  // Wskaźnik na aktualny znak w tekście
    uint16_t currentLineCount = 0;  // Licznik aktualnych linii na ekranie

    // Ustaw kolor tekstu i tła
    lcdSetTextColor(textColor, backgroundColor);

    while (*currentChar != '\0') {
        // Sprawdź, czy napotkano nową linię '\n'
        if (*currentChar == '\n') {
            // Jeżeli jest nowa linia, tylko zwiększ licznik linii
            currentLineCount++;
            currentChar++;  // Przejdź do kolejnego znaku
            continue;  // Kontynuuj bez dalszego przetwarzania tej linii
        }

        // Wyczyszczenie ekranu i resetowanie Y po osiągnięciu maksymalnych linii
        if (currentLineCount >= maxLines) {
            HAL_Delay(displayTimeMs);  // Czekaj przez określony czas
            lcdFillRGB(backgroundColor);  // Wyczyść ekran tłem
            currentY = startTextYposition;  // Zresetuj pozycję Y
            currentLineCount = 0;  // Zresetuj licznik linii
        }

        // Wyzerowanie bufora przed skopiowaniem danych
        memset(buffer, 0, sizeof(buffer));

        // Skopiuj maksymalną ilość znaków do bufora
        uint16_t charCount = 0;
        while (*currentChar != '\0' && *currentChar != '\n' && charCount < charsPerLine) {
            buffer[charCount++] = *currentChar++;
        }
        buffer[charCount] = '\0';  // Null-terminator

        // Wyświetl linię na ekranie
        lcdSetCursor(currentX, currentY);
        lcdPrintf("%s", buffer);

        // Przejdź do nowej linii po zakończeniu obecnej
        currentY += charHeight;
        currentLineCount++;
    }

    // Po wyświetleniu całego tekstu, poczekaj na określony czas i wyczyść ekran
    //HAL_Delay(displayTimeMs);
    DelayUS(displayTimeMs*1000);
    lcdFillRGB(backgroundColor);
}

void Send_ToLCDDisplay (char *string)
{
	lcdSetTextFont(&Font12);
	lcdDisplayTextWithScrollPage(string, 1000, 0,0, COLOR_WHITE, COLOR_BLACK);
}

void Debug(const char *textData)
{
	lcdSetTextFont(&Font12);

	lcdSetCursor(0, lcdGetHeight() - (lcdGetTextFont()->Height *2) - 2);
	lcdSetTextColor(COLOR_WHITE, COLOR_BLACK);

	char temp[100];
	memset(temp, ' ', sizeof(textData));
	lcdPrintf(temp);

	lcdSetCursor(0, lcdGetHeight() - (lcdGetTextFont()->Height *2) - 2);
	lcdPrintf("%s", textData);
	//lcdDisplayTextWithScrollPage(textData, 1000, 0, lcdGetHeight() - (lcdGetTextFont()->Height *2) - 2, COLOR_WHITE, COLOR_BLACK);

}

void PrintDateTime(const char *textData)
{
	lcdSetTextFont(&Font8);
	int length = strlen(textData);

	lcdSetCursor(lcdGetWidth() - (lcdGetTextFont()->Width *length)-1,0);
	lcdSetTextColor(COLOR_WHITE, COLOR_BLACK);
